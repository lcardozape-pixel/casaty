      ...
      <div className="relative z-20 max-w-[1700px] mx-auto px-4 w-full">
        <div className="max-w-4xl mb-8">
          ...
          >
            ...
            <h1 className="text-4xl md:text-6xl font-black text-white mb-4 leading-[1] tracking-tighter">
              El Hogar de Tus <br /> <span className="text-[#0040FF] drop-shadow-lg">Sueños</span> en Piura.
            </h1>
            <p className="text-base md:text-xl text-white/90 font-medium max-w-xl leading-relaxed">
        ...
        >
          {/* Tabs */}
          <div className="flex gap-2 mb-[-1px] relative z-10 ml-4">
             {(['Comprar', 'Alquilar', 'Proyectos'] as SearchTab[]).map((tab) => (
               <button
                 key={tab}
                 onClick={() => setActiveTab(tab)}
                 className={`px-6 py-3 rounded-t-2xl text-[11px] font-black uppercase tracking-widest transition-all ${
                   activeTab === tab 
                   ? 'bg-white text-[#0040FF] shadow-[0_-4px_20px_rgba(0,0,0,0.1)]' 
                   : 'bg-white/10 text-white backdrop-blur-md hover:bg-white/20 border-x border-t border-white/10'
                 }`}
               >
                 {tab}
               </button>
